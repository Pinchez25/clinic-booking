...
**User** — Extends `AbstractUser`. Email is the login field. Role is either `patient` or `doctor`. All users
authenticate via JWT.

**Doctor** — A `OneToOneField` to `User` holding shift data (`work_start`, `work_end`). No name, email, or phone on
`Doctor` itself — those live on `User`. This design prevents PII from leaking into doctor-related API responses by
accident.
...
2. Render pulls `main`, runs `build.sh`, performs a zero-downtime swap
...
- **Zero-downtime deploys**: Render swaps the new instance only after the health check at `/health/` passes

### Environment Variables on Render

Most variables are wired automatically via `render.yaml`. The following must be set manually in the Render dashboard:
...
**Section 2 (Implementation)**: Used AI for code generation across all apps. Every generated file was reviewed,
questioned, and often corrected. Notable catches during review: `timezone.utc` is not a Django attribute (it's
`datetime.UTC`), a shared `AuthRateThrottle` scope causing login and register to share a rate limit counter,
`ALLOWED_HOSTS` and `INSTALLED_APPS` being redundantly redefined in child settings files, and `is_active` on `Doctor`
being semantically ambiguous against `AbstractUser.is_active`.

**Section 3 (Deployment)**: Used AI to generate the GitHub Actions workflows, `render.yaml`, and Dockerfile. Reviewed
the `needs: ci` chaining logic and the `workflow_run` vs combined workflow trade-off before settling on a single
combined workflow.
...
does and realising the delegation was circumventing it. Fixed by removing the custom view entirely and letting
`TOKEN_OBTAIN_SERIALIZER` in `SIMPLE_JWT` settings do exactly what it was designed for.

### 4. Two decisions made without AI

**Keeping `AbstractUser` instead of `AbstractBaseUser`**: A code suggestion proposed setting `username = None` on the
model to remove the field. Before accepting it, I checked Django's source — `AbstractUser`'s manager has `username` as a
positional argument in `_create_user`, `create_user`, and `create_superuser`. Removing the field would cause a
`TypeError` at runtime. Kept `AbstractUser` and set `username=email` internally — the right call given the scope of this
project.
